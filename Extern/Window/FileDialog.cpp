...
void Windowing::Dialogs::FileDialog::Show(EExplorerFlags p_flags)
{
	...
	ofn.hwndOwner = NULL;  // If you have a window to center er, put its HANDLE here
	ofn.lpstrFilter =(LPWSTR) m_filter.c_str();
	ofn.lpstrFile = (LPWSTR)m_filepath.data();
	ofn.nMaxFile = MAX_PATH;
	ofn.lpstrTitle = (LPWSTR)m_dialogTitle.c_str();

	if (!m_initialDirectory.empty())
		ofn.lpstrInitialDir = (LPWSTR)m_initialDirectory.c_str();

	ofn.Flags = static_cast<DWORD>(p_flags);

	m_succeeded = m_callback((tagOFNA*)&ofn);
