...
#include <iostream>
 ...
 void opengl::ViewerCore::align_camera_center(
  ...
  const Eigen::MatrixXi& F)
{
 ...
 void opengl::ViewerCore::get_scale_and_shift_to_fit_mesh(
  const Eigen::MatrixXd& V,
  const Eigen::MatrixXi& F,
  float& zoom,
  ...
  Eigen::Vector3f& shift)
{
 ...
 void opengl::ViewerCore::clear_framebuffers()
{
  ...
  bool update_matrices)
{
 ...
 void opengl::ViewerCore::initialize_shadow_pass()
{
  // attach buffers
 ...
 void opengl::ViewerCore::deinitialize_shadow_pass()
{
  glBindFramebuffer(GL_FRAMEBUFFER, 0);
  glBindRenderbuffer(GL_RENDERBUFFER, 0);
}

 void opengl::ViewerCore::draw_shadow_pass(
  ViewerData& data,
  bool /*update_matrices*/)
{
  if (data.dirty)
  {
    data.updateGL(data, data.invert_normals, data.meshgl);
    data.dirty = opengl::MeshGL::DIRTY_NONE;
  }
  data.meshgl.bind_mesh();
 ...
 void opengl::ViewerCore::draw_buffer(
  ViewerData& data,
  bool update_matrices,
...
// Define uniforms for text labels
 void opengl::ViewerCore::draw_labels(
  ViewerData& data,
  const opengl::MeshGL::TextGL& labels
){
 ...
 void opengl::ViewerCore::set_rotation_type(
  const opengl::ViewerCore::RotationType & value)
{
  using namespace Eigen;
  using namespace std;
 ...
 void opengl::ViewerCore::set(unsigned int &property_mask, bool value) const
{
  if (!value)
    unset(property_mask);
  else
 ...
 void opengl::ViewerCore::unset(unsigned int &property_mask) const
{
  property_mask &= ~id;
}

 void opengl::ViewerCore::toggle(unsigned int &property_mask) const
{
  property_mask ^= id;
}

 bool opengl::ViewerCore::is_set(unsigned int property_mask) const
{
  return (property_mask & id);
}

 opengl::ViewerCore::ViewerCore()
{
  // Default colors
 ...
 void opengl::ViewerCore::init()
{
  delete_shadow_buffers();
  generate_shadow_buffers();
}

 void opengl::ViewerCore::shut()
{
  delete_shadow_buffers();
}

 void opengl::ViewerCore::delete_shadow_buffers()
{
 ...
 void opengl::ViewerCore::generate_shadow_buffers()
{
