	...
	namespace nurbs
	{
		...
		namespace util
		{
			...
			inline void global_interpolation(int degree, const std::vector<vec3>& throughpoints, RationalCurve& crv)
			{
				...
				// AverageKnotVector
				std::vector<float> knot_vector = average_knot_vector(degree, uk);
				// Solve Linear System
				...
				A[0][0] = 1.0;
				A[n][n] = 1.0;
				std::vector<std::vector<float>> right(size, std::vector<float>(3));
				...
				std::vector<std::vector<float>>  result = solve_linear_system(A, right);
